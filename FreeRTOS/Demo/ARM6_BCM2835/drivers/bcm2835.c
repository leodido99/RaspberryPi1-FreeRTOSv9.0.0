...
/* Adaptation of the lib for bare metal
 * - Don't use mman -> MAP_FAILED = NULL
 * - bcm2835_peripherals always pointing to physical addresses
 * - Removed any usage of functionalities provided by linux
 * - Removed usage of __sync_synchronize();
 * - Corrected -Wall warnings
 * - Corrected formating
 * */
 ...
 */
volatile uint32_t *bcm2835_gpio        = (uint32_t *)NULL;
volatile uint32_t *bcm2835_pwm         = (uint32_t *)NULL;
volatile uint32_t *bcm2835_clk         = (uint32_t *)NULL;
volatile uint32_t *bcm2835_pads        = (uint32_t *)NULL;
volatile uint32_t *bcm2835_spi0        = (uint32_t *)NULL;
volatile uint32_t *bcm2835_bsc0        = (uint32_t *)NULL;
volatile uint32_t *bcm2835_bsc1        = (uint32_t *)NULL;
volatile uint32_t *bcm2835_st	       = (uint32_t *)NULL;
...
uint32_t bcm2835_peri_read(volatile uint32_t* paddr)
{
	...
	if (debug)
	{
		PRINTUART("bcm2835_peri_read  paddr %08X\n", (unsigned) paddr);
		return 0;
	}
	else
	{
...
uint32_t bcm2835_peri_read_nb(volatile uint32_t* paddr)
{
	if (debug)
	{
		PRINTUART("bcm2835_peri_read_nb  paddr %08X\n", (unsigned) paddr);
		return 0;
	}
	else
	{
...
void bcm2835_peri_write(volatile uint32_t* paddr, uint32_t value)
{
	if (debug)
	{
		PRINTUART("bcm2835_peri_write paddr %08X, value %08X\n", (unsigned int) paddr,(unsigned int) value);
	}
	else
	{
		//__sync_synchronize();
...
void bcm2835_peri_write_nb(volatile uint32_t* paddr, uint32_t value)
{
	if (debug)
	{
		PRINTUART("bcm2835_peri_write_nb paddr %08X, value %08X\n",
				(unsigned int) paddr,(unsigned int) value);
	}
	else
	{
...
{
	/* Use processor timer to delay instead of nanosleep */
	bcm2835_delayMicroseconds(millis * 1000);
...
void bcm2835_delayMicroseconds(uint64_t micros)
{
	...
	if (debug)
	{
		...
		PRINTUART("bcm2835_delayMicroseconds %lld\n", micros);
		return;
	}
...
// Return mapped address on success, NULL otherwise.
...
int bcm2835_init(void)
{
	...
	//if (bcm2835_peripherals == NULL) goto exit;
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		if (bcm2835_gpio_eds(RPI_GPIO_P1_15))
		{
			...
			PRINTUART("low event detect for pin 15\n");
		}
